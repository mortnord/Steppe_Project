...
class Healing(Card):
    def __init__(self):
        ...
        self.sprite = Enumerators.Type_Card_Sprite.Healing.value

    def usage(self, card_nr, target_dwarf, nr_dwarf):

        Static_Data.get_list_of_people()[target_dwarf].health += self.value
        if Static_Data.get_list_of_people()[target_dwarf].health > Static_Data.get_list_of_people()[
            target_dwarf].max_health:
            Static_Data.get_list_of_people()[target_dwarf].health = Static_Data.get_list_of_people()[
                target_dwarf].max_health
