...
def harvest_grass(amount_harvester):  # verdien inn er hvor mange som kan høste (i dette tilfellet 1 per dverg)
    ...
    if Inventory.get_grass_amount() < Inventory.get_max_grass_amount():  # Så lenge vi ikke har max mengden så går
        # det fint
        if Static_Data.get_current_map().landscape.amount_of_grass > amount_harvester:  # Så lenge det er nok grass
            # på bakken til å det ikke blir tomt
            Static_Data.get_current_map().landscape.amount_of_grass -= amount_harvester  # så fjernes grass fra
            # landscapet
            ...
            amount_harvested = Static_Data.get_current_map().landscape.amount_of_grass  # Vis ikke tomt, men ikke nok
            # til en full høsting, så får vi alt vi kan.
    ...
    Turn_And_Background_Actions.turn_action.take_Action()  # Å høste grass tar tid, så da går det en runde, sjekk
    # implementation for detaljer
