...
class Combat:
    ...
    def player_use_card_round(self):  # Del 2 av kamp, spilleren gjør actions
        ...
        for x in range(len(Static_Data.get_list_of_people())):  # Skriv hvor mye liv ting har igjen
            print(
                "Dwarf " + str(x + 1) + " is ID " + str(Static_Data.get_list_of_people()[x].ID) + " with health " + str(
                    Static_Data.get_list_of_people()[x].health))

        while Static_Data.get_which_dwarf_to_attack() < len(
                Static_Data.get_list_of_people()):  # Kort-bruk-loopen, så lenge energien vi har brukt ikke overstiger
            ...
            if (len(Static_Data.get_list_of_people()) - Static_Data.get_which_dwarf_to_attack()) >= \
                    Static_Data.get_deck_list().hand[
                        use_card_nr].dwarfs_required:  # så lenge vi har nok energi til å bruke kortet, så går det fint.
                Static_Data.get_deck_list().hand[use_card_nr].usage(use_card_nr,
                                                                    Static_Data.get_which_dwarf_to_attack())  # Bruk kortet sitt effekt, avhengig av kortet som blir brukt
            self.check_for_deaths()  # Klarte vi å drepe goblins? la oss sjekke
