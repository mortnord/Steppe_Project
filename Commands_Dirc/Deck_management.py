...
def draw_cards_until_full(): #Her trekker vi kort til vi har fult
    while len(Static_Data.get_deck_list().hand) < Static_Data.get_deck_list().hand_max_amount: #trekk så lenge vi ikke har full hånd
...
def draw_card(): #Trekk-kort-metoden..må kanskje legge til while-loopen fra draw_cards_until_full
    Static_Data.get_deck_list().hand.append(Static_Data.get_deck_list().content.pop(0)) #Legg til første kort fra decket til hånda
    Static_Data.get_deck_list().hand_amount = len(Static_Data.get_deck_list().content) #Oppdatert mengden kort vi har i hånda
